    ...
    try {
        ...
        if ($conexion) {
            ...
            if ($resultado) {
                ...
                $resultado["autores"] = explode(",", $resultado["autores"]);
 ...
 * @param $fechaAlta Nueva fecha en la que se edita el libro
 ...
 * @throws Exception 
 */
function librosActualizar($titulo, $editorial, $genero, $isbn, $anhoPublicacion, $fechaAlta, $idLibro)
{
    try {
        ...
        if ($resultado) {
            if (!empty($autores)) {
                // Creamos el query.
                $query = "UPDATE autor_libro (idAutor, idLibro) WHERE idAutor= ";
                // Agregamos los valores de cada uno de los autores en la query.
